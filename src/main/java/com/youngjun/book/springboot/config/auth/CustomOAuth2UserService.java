...
import com.youngjun.book.springboot.config.auth.dto.OAuthAttributes;
import com.youngjun.book.springboot.config.auth.dto.SessionUser;
import com.youngjun.book.springboot.web.domain.user.User;
import com.youngjun.book.springboot.web.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException{
        ...
        //구글, 네이버 등 여러 로그인 연동 시에 플랫폼을 구분하기 위해 사용
        String registrationId = userRequest.getClientRegistration().getRegistrationId();

        //OAuth2 로그인 진행 시 key 가 되는 필드값을 선언
